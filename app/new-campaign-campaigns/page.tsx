...
export default function NewCampaignCampaignsPage() {
  ...
  console.log('🚨 EMERGENCY ADMIN CHECK (CLOUDLEA):', { 
    isEmergencyAdmin, 
  ...
  useEffect(() => {
    console.log('🔍 First useEffect - Auth check (CLOUDLEA):', { user: user?.email, loading, isAdminAuth, isEmergencyAdmin })
    
    // Skip all auth checks if emergency admin
    if (isEmergencyAdmin) {
      console.log('🚨 SKIPPING AUTH CHECK - Emergency admin detected (CLOUDLEA)')
      return
    }
  ...
  useEffect(() => {
    console.log('🔍 Second useEffect - Permission check trigger (CLOUDLEA):', { 
      hasUser: !!user, 
      loading, 
      isAdminAuth, 
      isRegularUserAuth,
    ...
    if (isEmergencyAdmin) {
      console.log('🚨 SKIPPING PERMISSION CHECK - Emergency admin detected (CLOUDLEA)')
      return
    }
    ...
    try {
      ...
      if (isAdminAuth || isRegularUserAuth) {
        ...
        if ((isAdminAuth && (email === 'adimahna@gmail.com' || email === 'adimstuff@gmail.com')) || 
          ...
          console.log('✅ ADMIN BYPASS: Setting admin permissions directly for Cloudlea campaigns')
          setUserPermissions({
            isAdmin: true,
            allowedCampaigns: ['roger', 'reachify', 'prusa', 'cloudlea', 'unified', 'new-campaign']
          })
      ...
      if (response.ok) {
        ...
        // Check if user has access to Cloudlea campaigns
        if (!data.isAdmin && !data.allowedCampaigns?.includes('new-campaign') && !data.allowedCampaigns?.includes('cloudlea') && !data.allowedCampaigns?.includes('prusa')) {
  ...
  if (isEmergencyAdmin) {
    console.log('🚨 EMERGENCY ADMIN RENDER BYPASS (CLOUDLEA)')
    return (
      <div className="flex min-h-screen bg-white dark:bg-slate-950">
        ...
        <div className="flex-1">
          <DashboardHeader />
          <main className="p-8">
            <UnifiedCampaignsDashboard defaultCategory="cloudlea" title="Cloudlea Campaign Dashboard" />
          </main>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-300">Loading Cloudlea campaigns...</p>
        </div>
      </div>
    )
  }
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950">
        ...
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 px-6 py-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
              ...
              <Button 
                variant="ghost" 
                size="sm" 
                className="rounded-xl hover:bg-red-100 dark:hover:bg-red-900 text-slate-600 dark:text-slate-300 hover:text-red-600 dark:hover:text-red-400"
                onClick={() => {
                  localStorage.removeItem('user')
                  router.push('/signin')
                }}
